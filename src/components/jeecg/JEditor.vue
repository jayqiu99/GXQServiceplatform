...
<script>
  ...
  import { uploadAction,getimagesHttpUrl } from '@/api/manage'
      ...
      return {
        ...
        init: {
          ...
          images_upload_handler: (blobInfo, success) => {
            ...
            formData.append('systype', "publicImg");
            formData.append("jeditor","1");
            uploadAction("/base/upload", formData).then((res) => {
              if (res.success) {
                if(res.message == 'local'){
                  console.log("测试上次",blobInfo.base64());
                  console.log("测试下次",res);
                  const img = 'data:image/jpeg;base64,' + blobInfo.base64()
                  success(img)
                }else{
                  console.log("3222222",res);
                  let img = getimagesHttpUrl(res.result)
                  success(img)
                }
              }
            })
