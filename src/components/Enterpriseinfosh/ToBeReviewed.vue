<template>
  <div v-loading="temloading" element-loading-text="拼命上传中......" element-loading-spinner="el-icon-loading"
    ...
    <a-card :bordered="false">
      ...
      <div class="table-page-search-wrapper">
        <div style="margin-left: 5%">
          <a-form layout="inline" @keyup.enter.native="searchQuery">
            <a-row :gutter="12">
              ...
              <!-- <a-col :md="8" :sm="8">
                <a-form-item label="是否错误" :labelCol="{ span: 6 }" :wrapperCol="{ span: 14, offset: 1 }">
        ...
        <div class="table-operator" style="border-top: 5px">
          ...
          <a-upload name="file" style="margin-left: 8px; display: inline-block" :multiple="false" @change="changeFile"
            accept=".excle, .xlsx" :customRequest="customRequest">
            <!-- <a-button type="primary" icon="upload">导入Excel</a-button> -->
          </a-upload>
          <a-button type="primary" style="margin-left: 8px; display: inline-block" @click="exportFile">导出查询数据</a-button>
           
          <!-- <a-button type="primary" style="margin-left: 10px; display: inline-block" @click="DownFile" icon="download"
            :size="size">
            下载导入模板
          </a-button> -->
        ...
        <div>
          ...
          <a-table ref="table" rowKey="id" size="middle" :columns="columns" :dataSource="dataSource"
            :scroll="{ x: 1500, y: 500 }" :pagination="ipagination" :loading="loading" bordered
...
<script>
  import { editresetenterprise, editenable, toberetList, editexamine } from '@/api/api'
  ...
  export default {
    ...
    components: { UserModal, ExamineItemList },
    data() {
      return {
        listforlebel: [], //企业下拉
        downloadhttp:'../../../public/测试文件.xlsx',
    ...
    methods: {
      ...
      DownFile() {
        ...
        window.location.href="../../../public/测试文件.xlsx"
      },
      exportFile() {
        var that = this
        ...
        console.log('iserror', tiserror)

        this.$http({
          url:
            '/hall/enterprise/exportXlsEnterpriseListTemporary?area=' +
            area +
            '&trade=' +
            trade +
            '&name=' +
      ...
      handleEdit(res) {
        ...
        this.$refs.modalForm.edit(res)
      },
      examineFormclick(resobj) {
        resobj.examinestate = 0
        console.log('审核对象', resobj)
        if (resobj.iserror == 0 || resobj.errorReason == '该企业已存在') {
          this.$refs.examineForm.edit(resobj)
        } else if (resobj.iserror == 1 && resobj.errorReason != '该企业已存在') {
      ...
      loadData(arg) {
        ...
        toberetList(params).then((res) => {
          if (res.success) {
            ...
            console.log('企业信息待审核', res.result.records)
      ...
      customRequest(data) {
        ...
        // console.log("上传步骤22222");
        const formData = new FormData()
        formData.append('file', data.file)
        formData.append('token', 'token')
        this.axios({
          method: 'post',
          url: '/hall/enterprise/importExcel',
          headers: {},
          params: {},
          data: formData,
        })
          .then((response) => {
            ...
            if (response.success == true) {
              // console.log("上传状态1",response)
              this.$message.success(`企业信息导入成功！`)
            } else {
              // console.log("上传状态2",response)
