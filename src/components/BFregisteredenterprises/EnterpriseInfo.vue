<template>
  <div>
    <a-drawer title="详细信息" :width="800" :visible="visible" :body-style="{ paddingBottom: '80px' }" @close="onClose">
      ...
      <a-row style="width: 100%;" :gutter="[0,32]">
        <a-col :span="24/2">
          <h3>联系人：{{formData.companyPrincipal}}</h3>
        </a-col>
      </a-row>
      <a-row style="width: 100%;" :gutter="[0,32]">
        <a-col :span="24/2">
      ...
      <a-row style="width: 100%;" :gutter="[0,32]">
        ...
        <a-col :span="24/2" :pull="8">
          <a-upload action list-type="picture-card" :file-list="fileListLogo" @preview="handlePreview"
            @change="handleChange" disabled></a-upload>
      ...
      <a-row style="width: 100%;" :gutter="[0,32]">
        ...
        <a-col :span="24/2" :pull="8">
          <a-upload action list-type="picture-card" :file-list="fileListBusinessLicenseAddress"
            @preview="handlePreviewBusinessLicenseAddress" @change="handleChangeBusinessLicenseAddress" disabled>
          </a-upload>
          <a-modal :visible="previewVisibleBusinessLicenseAddress" :footer="null"
      ...
      <a-row style="width: 100%;" :gutter="[0,32]">
        ...
        <a-col :span="24/2" :pull="8">
          ...
          <a-modal :visible="previewVisibleBusinessLicenseAddress" :footer="null"
            @cancel="handleCancelBusinessLicenseAddress">
            <img alt="委托书" style="width: 100%" :src="imgsrc+formData.businessLicenseAddress" />
          </a-modal>
        </a-col>
      </a-row>
      <a-row style="width: 100%;" :gutter="[0,32]">
        ...
        <a-col :span="24/2" :pull="8">
          ...
          <a-modal :visible="previewVisibleBusinessLicenseAddress" :footer="null"
            @cancel="handleCancelBusinessLicenseAddress">
            <img alt="用工备案表" style="width: 100%" :src="imgsrc+formData.businessLicenseAddress" />
          </a-modal>
        </a-col>
      </a-row>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      handleEdit() {
        this.$refs.modalForm.edit(this.restobj)
        this.visible = false
      },
      showDrawer(record) {
        this.visible = true
        ...
        getAction('/hall/enterprise/list', { id: record.enterpriseid }).then((res) => {
          ...
          this.wtsimgur[0].url=getUrl() + this.formData.poweraTtorneyAddress
          this.babimgur[0].url=getUrl() + this.formData.employmentFilingAddress
