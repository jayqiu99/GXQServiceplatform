<template>
  <a-card :bordered="false">
    ...
    <div class="table-page-search-wrapper" id="tableinfo">
      ...
      <!-- 操作按钮区域 -->
        <div class="table-operator" style="border-top: 5px">
          <a-button @click="toaddregisck" type="primary" icon="plus">添加报名企业</a-button>
        </div>
      <a-table
        ref="table"
        bordered
      ...
      >
        <span slot="action" slot-scope="text, record">
          <a @click="detail(record)"><a-icon type="setting" />发布岗位</a>
        </span>
    ...
    <addent-moadl ref="addentform" @ok="modalFormOk"></addent-moadl>
  </a-card>
</template>
...
import EnterpriseInfo from './modules/SeePosition.vue'
import AddModal from './modules/SeePosition'
import AddentMoadl from './modules/AddenterpriseModal.vue'
import Vue from 'vue'
export default {
  name: 'DictList',
  mixins: [JeecgListMixin],
  components: { EnterpriseInfo, AddModal, JInput, InvitationJob,AddentMoadl },
  data() {
    return {
      visible: false,
      ...
      url: {
        list: '/hall/entryenterprise/queryEntryEnterpriseListbyjobfair',
  ...
  methods: {
    toaddregisck(){
      console.log("添加报名企业+++++++++++",this.queryParam.jobfairId);
      // addentform
      this.$refs.addentform.edit(this.queryParam.jobfairId)
    },
    unexconfirm(reId) {
      let that = this
    ...
    detail(record) {
      ...
      this.$refs.enterpriseinfo.edit(record)
