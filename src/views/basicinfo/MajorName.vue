<template>
  <a-card :bordered="false">
          ...
          <a-col :md="7" :sm="8" :push="2">
            <a-form-item label="专业类型" :labelCol="{ span: 6 }" :wrapperCol="{ span: 14, offset: 1 }">
              <a-select v-model="queryParam.majortype" show-search :filter-option="filterOption">
                <a-select-option v-for="d in marjortypelist" :key="d.id" :value="d.id">{{
                  d.dataName
                }}</a-select-option>
              </a-select>
            </a-form-item>
    ...
    <major-name-modal ref="modalForm" @ok="modalFormOk"></major-name-modal>
  </a-card>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      url: {
        typelist: '/base/list?type=majortype&pageNo=1&pageSize=1000',
        list: '/base/parentlist?type=majorname',
        delete: '/base/delete',
      },
    }
  },
