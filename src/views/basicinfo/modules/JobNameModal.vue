      ...
      <a-form :form="form">
        <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="专业类型">
          <a-select v-decorator.trim="['parentid', validatorRules.parentid]" show-search :filter-option="filterOption">
            <a-select-option v-for="d in jobtypelist" :key="parseInt(d.id)" :value="parseInt(d.id)">{{
              d.dataName
            }}</a-select-option>
          </a-select>
        </a-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      validatorRules: {
        dataName: { rules: [{ required: true, message: '请输入职位类型名称!' }] },
        parentid: { rules: [{ required: true, message: '请选择职业类型!' }] },
      },
      jobtypelist: [],
      url: {
        jobtypelist: '/base/list?type=jobtype&pageNo=1&pageSize=1000',
        add: '/base/add',
        edit: '/base/editById',
      },
    }
  ...
  methods: {
    ...
    getMarjorTypeList() {
      getAction(this.url.jobtypelist).then((res) => {
        this.jobtypelist = res.result.records
      })
    },
