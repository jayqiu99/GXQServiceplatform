...
<script>
  ...
  export default {
    ...
    methods:{
          ...
          if(this.isMultiDepart===false){
            ...
            putAction("/sys/selectDepart",obj).then(res=>{
              if(res.success){
                ...
                userInfo.csitem="测试用户字段";
                this.$store.commit('SET_INFO', userInfo);
                // console.log("登录获取用户信息",res.result.userInfo);
      ...
      handleDepartChange(e,res){
        // console.log("部门信息",this.departList);
        // console.log("部门信息",res.data.key);
        localStorage.removeItem('departId')
        localStorage.setItem("departId", res.data.key); 
        this.validate_status2 = ''
        this.orgCode = e
      }
    }
