  ...
  <div class="main user-layout-register">
    ...
    <a-form ref="formRegister" :autoFormCreate="(form)=>{this.form = form}" id="formRegister">
      <a-form-item
        ...
        :fieldDecoratorOptions="{rules: [{ required: true, message: '企业名称不能为空'}, { validator: this.Enterprisename }], validateTrigger: ['change', 'blur'], validateFirst: true}"
      >
        <a-input size="large" type="text" autocomplete="false" placeholder="请输入企业名称"></a-input>
      </a-form-item>
      <a-form-item
        fieldDecoratorId="creditCode"
        :fieldDecoratorOptions="{rules: [{ required: true, message: '企业信用代码不能为空'}, { validator: this.creditCode }], validateTrigger: ['change', 'blur'], validateFirst: true}"
      >
        <a-input size="large" type="text" autocomplete="false" placeholder="请输入企业信用代码"></a-input>
      </a-form-item>
      <a-form-item
        fieldDecoratorId="contactsName"
        :fieldDecoratorOptions="{rules: [{ required: true, message: '联系人不能为空'}, { validator: this.contactsName }], validateTrigger: ['change', 'blur'], validateFirst: true}"
      >
      ...
      >
        <a-input size="large" placeholder="请输入联系电话/登录账号">
      ...
      <a-form-item
        fieldDecoratorId="enterpriseType"
        :fieldDecoratorOptions="{rules: [{ required: true, message: '请选择企业类型'}, { validator: this.enterpriseType }], validateTrigger: ['change', 'blur'], validateFirst: true}"
      >
        <a-select
          show-search
        ...
        >
          ...
          <a-select-option v-for="d in enterprisetypes" :key="d.value" :value="d.value">
            {{
            d.name
            }}
          </a-select-option>
...
export default {
  ...
  methods: {
    ...
    enterpriseType(rule, value, callback) {
      var params = {
    ...
    Enterprisename(rule, value, callback) {
      var params = {
    ...
    contactsName(rule, value, callback) {
      var params = {
        contactsName: value
      }
      callback()
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          var register = {
            ...
            companyPrincipal: values.contactsName,
            password: values.password,
            email: values.email,
            enterpriseType:values.enterpriseType,
            phone: values.mobile,
            createBy: values.contactsName,
            createTime: this.getNowFormatDate()
            // smscode: values.captcha,
          }
          console.log('注册信息2', register)
          postAction('/hall/enterprise/registered', register).then(res => {
            if (!res.success) {
